...
app.post('/api/generate', requireUser, async (req,res)=>{
  ...
  const metaInstruction = metaRequested ? `\nAdemás crea un paquete de META ADS para Facebook e Instagram con: meta.primary_texts (array de 3 textos principales, distintos entre sí, listos para anuncios), meta.headlines (array de 5 titulares de máximo 40 caracteres), meta.descriptions (array de 3 descripciones cortas), meta.cta (un CTA breve) y meta.recommended_format (Feed, Stories o Reels). Respeta las políticas publicitarias: no hagas atributos personales sensibles, no prometas resultados garantizados y no inventes descuentos o características.` : `\nNo generes el paquete de Meta Ads porque el usuario lo desactivó.`;
  const content=[{type:'input_text',text:`Eres el director creativo de una agencia de performance marketing. Crea una campaña para este producto.\nProducto: ${data.name||''}\nPrecio: ${data.price||''}\nDescripción: ${data.description||''}\nPúblico: ${data.audience||''}\nObjetivo: ${data.goal||'ventas'}\n${metaInstruction}\nDevuelve SOLO JSON válido con estas claves: headline, emotional, offer, direct, whatsapp, reel, audiences (array de 3 strings), cta, meta. Si Meta Ads está desactivado, meta debe ser null. No inventes características técnicas ni descuentos que no estén dados. Escribe en español latino. Sé persuasivo sin prometer resultados garantizados.` }];
